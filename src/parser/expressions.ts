...
export function parseAssignmentExpression(
  ...
  if ((token & Token.IsAssignOp) === Token.IsAssignOp) {
    ...
    const operator = KeywordDescTable[token & 0xff] as ESTree.AssignmentOperator;
...
export function parseAsyncExpression(
  ...
  if (parser.newLine === 0) {
    ...
    if (parser.token === Token.FunctionKeyword) {
      if (allowLHS === 0) report(parser, Errors.UnexpectedToken, KeywordDescTable[parser.token & 0xff]);
...
export function parseArrowFunction(
  ...
  if (expression) {
    ...
    if ((parser.token & Token.IsBinaryOp) === Token.IsBinaryOp && parser.newLine === 0) {
      report(parser, Errors.UnexpectedToken, KeywordDescTable[parser.token & 0xff]);
...
export function parseParenthesizedExpression(
  ...
  while ((parser.token as Token) !== Token.RightParen) {
    ...
    if (token & (Token.Keyword | Token.FutureReserved | Token.IsIdentifier)) {
      ...
      if (isSequence && ((parser.token as Token) === Token.RightParen || (parser.token as Token) === Token.Comma)) {
      ...
      if (isSequence && ((parser.token as Token) === Token.Comma || (parser.token as Token) === Token.RightParen)) {
        expressions.push(expr);
      }
  ...
  if (parser.token === Token.Arrow) {
    if (context & (Context.InAwaitContext | Context.Module) && conjuncted & Flags.SeenAwait) {
      report(parser, Errors.AwaitInParameter);
    }
...
export function parseUpdateExpression(
  ...
  const operator = KeywordDescTable[parser.token & 0xff] as ESTree.UpdateOperator;
...
export function parseUnaryExpression(
  ...
  if (inNew === 1) report(parser, Errors.InvalidNewUnary, KeywordDescTable[parser.token & 0xff]);
  ...
  return context & Context.OptionsLoc
    ? {
        ...
        operator: KeywordDescTable[operator & 0xff] as ESTree.UnaryOperator,
        argument: arg,
        prefix: true,
        start,
        end: parser.endIndex,
      ...
      if (token & (Token.Keyword | Token.FutureReserved | Token.IsIdentifier)) {
        ...
        if (parser.token === Token.Assign) {
          ...
          if ((parser.token as Token) !== Token.Comma && (parser.token as Token) !== Token.RightBracket) {
            if ((parser.token as Token) !== Token.Assign) conjuncted |= Flags.NotDestructible;
...
export function parseClassElementList(
  ...
  parser.flags =
    (conjuncted | Flags.StrictEvalArguments | Flags.HasStrictReserved) ^
      ...
      if (token & (Token.Contextual | Token.Keyword | Token.FutureReserved | Token.IsIdentifier)) {
        ...
        if (
          ...
          if (parser.token === Token.Assign) {
            ...
            if (inGroup === 1 && token === Token.AwaitKeyword) conjuncted |= Flags.SeenAwait;
          ...
          if (parser.token & (Token.Contextual | Token.Keyword | Token.FutureReserved | Token.IsIdentifier)) {
            ...
            if ((parser.token as Token) === Token.Comma || (parser.token as Token) === Token.RightBrace) {
              ...
              if (((parser.token as Token) & Token.IsAssignOp) === Token.IsAssignOp) {
                operator = KeywordDescTable[parser.token & 0xff] as ESTree.AssignmentOperator;
        ...
        if ((parser.token as Token) === Token.Colon) {
          ...
          if ((parser.token as Token) & (Token.Keyword | Token.FutureReserved | Token.IsIdentifier)) {
          ...
          state |= PropertyKind.Method;
          conjuncted |= parser.flags & Flags.SeenAwait ? Flags.SeenAwait : 0;
          value = parseMethodDefinition(parser, context, state);
          conjuncted |= Flags.NotDestructible;
        } else {
        ...
        report(parser, Errors.UnexpectedToken, KeywordDescTable[token & 0xff]);
      }
