(function(global, factory) {
  ...
  function parseBinaryExpression(parser, context, inGroup, minPrec, l, curStart, curLine, curColumn, left) {
    ...
    while ((l & 135266304) > 0) {
      ...
      if (((l | t) & 0b01000000100000000000000000000000) > 1073741824) {
        report(parser, 45);
      }
      nextToken(parser, context, 1);
  ...
  function parseAsyncArrowOrCallExpression(
    ...
    while (parser.token !== 17) {
      ...
      if ((token & 0b00000000001001110000000000000000) > 0) {
        ...
        consume(parser, context, 17, 0);
        parser.flags =
          ((parser.flags | 0b00000000000000000000000000011110) ^ 0b00000000000000000000000000011110) | mutualFlag | 8;
        parser.assignable = 0;
        return context & 2
          ? {
              type: 'CallExpression',
  ...
  function parseClassDeclaration(parser, context, scope, flags) {
    ...
    if (parser.token & (131072 | 262144 | 2162688) && parser.token !== 131159) {
      ...
      if ((flags & 0b00000000000000000000000000000001) === 0) report(parser, 32, 'Class');
    }
  ...
  function parseVariableDeclarationListAndDeclarator(parser, context, scope, kind, origin) {
    ...
    while (parser.token !== 19) {
      const { start, line, column } = parser;
      type =
        kind | ((parser.token & 0b00000010000000000000000000000000) === 0b00000010000000000000000000000000 ? 1024 : 0);
  ...
  function parseForStatementWithVariableDeclarations(
    ...
    if ((kind & 0b00000000000000000000000000110010) > 0) {
      ...
      while (parser.token !== 19) {
        ...
        type =
          kind |
          ((parser.token & 0b00000010000000000000000000000000) === 0b00000010000000000000000000000000 ? 1024 : 0);
        let id;
        let init = null;
        if ((token & 0b00000000001001110000000000000000) !== 0) {
  ...
  function parseExpressionOrLabelledStatement(parser, context, scope, origin, labels, nestedLabels, allowFuncDecl) {
    ...
    return parseExpressionStatement(
      parser,
      context,
      parser.token === 19 ? parseSequenceExpression(parser, context, expr, start, line, column) : expr,
      start,
      line,
      column
    );
