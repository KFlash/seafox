...
function scanIdentifier(parser, context, source, char, maybeKeyword) {
  ...
  if (char > 90) return scanIdentifierSlowPath(parser, context, source, value, maybeKeyword);
...
function scanRegularExpression(parser, context, source) {
  ...
  while (isIdentifierPart(char)) {
    switch (char) {
      case 103:
        if (mask & 2) report(parser, 14, 'g');
        mask |= 2;
        break;
      case 105:
        if (mask & 1) report(parser, 14, 'i');
        mask |= 1;
        break;
      case 109:
        ...
        break;
      case 117:
        if (mask & 16) report(parser, 14, 'g');
        mask |= 16;
        break;
      case 121:
        if (mask & 8) report(parser, 14, 'y');
        mask |= 8;
        break;
      case 115:
        if (mask & 12) report(parser, 14, 's');
        mask |= 12;
        break;
      default:
        report(parser, 13);
  ...
  if (context & 8) parser.tokenRaw = source.slice(parser.start, parser.index);
...
function scanNumber(parser, source, char, skipSMI) {
  ...
  if (skipSMI === 0) {
    ...
    if (digit >= 0 && char !== 46 && (CharTypes[char] & (2 | 1)) === 0) {
      parser.tokenValue = value;
      return 1572866;
    }
  }
...
function addBlockName(parser, context, scope, name, kind, origin) {
  ...
  if (value && (value & 8) !== 8) {
    ...
    } else if ((context & 16) !== 16 && value & 4 && origin & 2);
    else {
      report(parser, 22, name);
    }
  }
  if (scope.type & 128 && scope.parent['#' + name] && (scope.parent['#' + name] & 8) !== 8) {
    report(parser, 22, name);
  }
  ...
  if (scope.type & 64) {
    if (scope.parent['#' + name] & 768) report(parser, 23, name);
  }
  scope['#' + name] = kind;
}
function addVarName(parser, context, scope, name, kind) {
  ...
  while (currentScope && (currentScope.type & 256) === 0) {
    ...
    if (value & 244) {
      if ((context & 16) !== 16 && (context & 1024) === 0 && ((kind & 128 && value & 6) || (value & 128 && kind & 6)));
      else {
        report(parser, 22, name);
      }
    }
...
function isValidIdentifier(context, t) {
  if (context & (1024 | 2097152)) {
    if (context & 2048 && t === 3211376) return false;
    if (context & 2097152 && t === 3473517) return false;
    return (t & 2162688) === 2162688 || (t & 65536) === 65536;
  }
  return (t & 2162688) === 2162688 || (t & 65536) === 65536 || (t & 262144) === 262144;
}
...
function isStrictReservedWord(parser, context, t, inGroup) {
  ...
  if (t === 3473517 && context & 2097152) report(parser, 31, 'yield');
  return (t & 131072) === 131072 || (t & 262144) === 262144 || t == 2162808;
}
...
function parseExpressions(parser, context, inGroup) {
  ...
  return parser.token === 19 ? parseSequenceExpression(parser, context, property, start, line, column) : property;
}
...
function parseBinaryExpression(parser, context, inGroup, minPrec, logical, curStart, curLine, curColumn, left) {
  ...
  while ((logical & 135266304) === 135266304) {
    t = parser.token;
    if ((t & prec) + ((t === 135318585) << 8) <= minPrec) return left;
    if (((logical & 8388608) | (t & 1073741824) | ((t & 8388608) | (logical & 1073741824))) > 1073741824) {
      report(parser, 45);
    }
    nextToken(parser, context, 1);
...
function parseMemberExpression(parser, context, expr, isOptional, isShortCircuited, inGroup, start, line, column) {
  ...
  switch (parser.token) {
    ...
    case 1048586:
    case 1048585: {
      if (isShortCircuited === 1) report(parser, 6);
      parser.assignable = 0;
      const quasi =
        parser.token === 1048585
    ...
    case 25: {
      if (isOptional === 1) report(parser, 0);
      parser.assignable = 0;
      nextToken(parser, context, 0);
      isOptional = 1;
      if ((parser.token & (65536 | 131072 | 262144 | 2162688)) !== 0) {
...
function parsePrimaryExpression(parser, context, kind, inNew, allowLHS, canAssign, inGroup, start, line, column) {
  ...
  if ((token & 2162688) === 2162688) {
    switch (token) {
      case 3473517:
        if (inGroup === 1) parser.flags |= 1024;
...
function parseAsyncExpression(parser, context, inNew, allowLHS, canAssign, curStart, curLine, curColumn) {
  ...
  if (parser.newLine === 0) {
    if (parser.token === 1179738) {
      if (allowLHS === 0) report(parser, 86, KeywordDescTable[parser.token & 255]);
...
function parseAsyncArrowOrCallExpression(
  ...
  parser.flags = (parser.flags | 1024 | 256) ^ (256 | 1024);
  let expr = null;
  let conjuncted = 0;
  const params = [];
  while (parser.token !== 17) {
    ...
    if (token & (2162688 | 131072 | 262144)) {
      ...
      if (parser.token !== 17 && parser.token !== 19) {
        if (conjuncted & 16) report(parser, 64);
        expr = parseMemberExpression(parser, context, expr, 0, 0, 0, start, line, column);
        conjuncted |= 8;
      ...
      conjuncted |= (parser.token === 17 ? 0 : 8) | (parser.flags | 256);
    } else {
  ...
  consume(parser, context, 17, 0);
  conjuncted |= (parser.flags & 1024 ? 1024 : 0) | (parser.flags & 2048 ? 2048 : 0);
  if (parser.token === 11) {
    if (parser.flags & 2048) report(parser, 37);
  ...
  if (conjuncted & 16) report(parser, 58);
  parser.assignable = 0;
  return context & 2
    ? {
        type: 'CallExpression',
...
function parseArrowFunctionAfterParen(
  ...
  parser.flags = ((parser.flags | 1024 | 2048 | 30) ^ (30 | 1024 | 2048)) | conjuncted;
  if (canAssign === 0) report(parser, 73);
  let i = params.length;
  while (i--) {
...
function parseParenthesizedExpression(parser, context, inGroup, canAssign, kind, origin, curStart, curLine, curColumn) {
  ...
  parser.flags = (parser.flags | 1024 | 2048 | 256) ^ (256 | 1024 | 2048);
  context = (context | 8192) ^ 8192;
  let expr = [];
  if (parser.token === 17) {
    if (canAssign === 0) report(parser, 73);
...
function parseUnaryExpression(parser, context, inGroup, inNew, allowLHS, start, line, column) {
  if (allowLHS === 0) report(parser, 0);
  if (inNew === 1) report(parser, 48, KeywordDescTable[parser.token & 255]);
  ...
  while (parser.token !== 21) {
    if (consumeOpt(parser, context, 19, 1)) {
      ...
      if (token & (131072 | 262144 | 2162688)) {
        ...
        if (parser.token !== 19 && parser.token !== 21) {
          left = parseAssignmentExpression(parser, context, isPattern, 0, 0, left, start, line, column);
          if ((kind & (8 | 1)) === 0 && token === 1048588) conjuncted |= 8;
        } else if (parser.assignable === 0) {
          conjuncted |= 8;
        } else if (token === 1048588) {
          conjuncted |= parser.assignable === 1 && kind & (8 | 1) ? 4 : 8;
        }
      }
      elements.push(left);
      if (parser.token !== 19) break;
...
function parseFunctionLiteral(parser, context, scope, id, firstRestricted, flags, type, isMethod, start, line, column) {
  ...
  const body = parseFunctionBody(
    parser,
    (context | 268435456 | 134217728 | 131072) ^ (268435456 | 134217728 | 131072),
    {
      parent: scope,
      type: 128,
      scopeError: void 0
...
function parseFunctionBody(parser, context, scope, firstRestricted, flags, scopeError) {
  ...
  if (parser.token !== 16777232) {
    ...
    if (context & 1024) {
      ...
      if (parser.flags & 32) report(parser, 27);
      if (parser.flags & 64) report(parser, 26);
    }
  }
  parser.flags = (parser.flags | 32 | 64 | 128 | 1024 | 2048) ^ (32 | 64 | 128 | 1024 | 2048);
  ...
  parser.flags = (parser.flags | 256 | 1024 | 2048) ^ (256 | 1024 | 2048);
  return context & 2
    ? {
        type: 'BlockStatement',
        body,
...
function parseClassDeclarationOrExpressionRest(
  parser,
  context,
  inheritedContext,
  id,
...
function parseClassElementList(
  ...
  if (isComputed === 0) {
    if (parser.tokenValue === 'constructor') {
      ...
      type |= 64;
    } else if (type & (32 | 384 | 8 | 16) && parser.tokenValue === 'prototype') {
      report(parser, 55);
    }
  }
  const value =
  ...
  const kind = isStatic === 0 && type & 64 ? 'constructor' : type & 128 ? 'get' : type & 256 ? 'set' : 'method';
  return context & 2
    ? {
...
function parseGetterSetter(parser, context, kind) {
  ...
  if (parser.token !== 17) {
    ...
    const modifierFlags = (kind & 64) === 0 ? 31981568 : 14680064;
    context = ((context | modifierFlags) ^ modifierFlags) | ((kind & 88) << 18) | 100925440 | ((context | 8192) ^ 8192);
    let argCount = 0;
    let left;
    ...
    while (parser.token !== 17) {
      ...
      if (parser.token & (131072 | 262144 | 2162688)) {
        if ((context & 1024) !== 1024) {
          parser.flags |= ((token & 262144) === 262144 ? 64 : 0) | (isEvalOrArguments(tokenValue) ? 32 : 0);
        }
    ...
    if (scope && scope.scopeError !== void 0) reportScopeError(scope.scopeError);
    if (isSimpleParameterList === 1) parser.flags |= 256;
  } else if (kind & 256) {
    report(parser, 38, 'Setter', 'one', '');
  }
  consume(parser, context, 17, 0);
...
function parseObjectLiteralOrPattern(
      ...
      if (token & (65536 | 131072 | 262144 | 2162688)) {
        ...
        if (parser.token === 19 || parser.token === 16777232 || parser.token === 67108896) {
          ...
          if (parser.token === 67108896) {
            ...
            if (inGroup === 1 && token === 3211376) conjuncted |= 2048;
            value = key;
          }
          ...
          if (parser.token & (65536 | 131072 | 262144 | 2162688)) {
            ...
            if (parser.token === 19 || parser.token === 16777232) {
              if (parser.assignable === 0) conjuncted |= 8;
          ...
          value =
            state & 384 ? parseGetterSetter(parser, context, state) : parseMethodDefinition(parser, context, state);
        ...
        if (parser.token === 22) {
          ...
          if (parser.token & (131072 | 262144 | 2162688)) {
            ...
            value = parseMemberExpression(parser, context, value, 0, 0, 0, start, line, column);
            if ((parser.token & 67108864) === 67108864) {
              conjuncted |= parser.assignable === 0 ? 8 : token === 67108896 ? 0 : 4;
              operator = KeywordDescTable[parser.token & 255];
              nextToken(parser, context, 1);
              value = parseAssignmentOrPattern(parser, context, isPattern, 0, value, operator, start, line, column);
  ...
  if (prototypeCount > 1) conjuncted |= 512;
  const node =
    context & 2
      ? {
...
function parseSpreadOrRestElement(
  ...
  if (token & (131072 | 262144 | 65536 | 2162688)) {
    ...
    if (token & 33554432) {
      ...
      if (token !== 67108896 && token !== closingToken && token !== 19) {
        ...
        conjuncted |= closingToken === 16777232 && token !== 67108896 ? 8 : parser.flags;
      }
    } else {
      conjuncted |= 4;
  ...
  if (parser.token !== closingToken) {
    if (kind & 1) conjuncted |= isAsync ? 8 : 4;
    if (parser.token === 67108896) {
      if (conjuncted & 8) report(parser, 60);
      nextToken(parser, context, 1);
...
function parseImportExpression(parser, context, start, line, column) {
  ...
  if (parser.token === 17) report(parser, 0);
  if (parser.token === 15) report(parser, 0);
...
function parseLabelledStatement(
  ...
  const body =
    context & (16 | 1024) || parser.token !== 1179738
...
function parseForStatement(parser, context, scope, labels) {
  ...
  if ((parser.token & 4194304) === 4194304) {
    ...
    if (parser.token === 4259957) {
      if (parser.assignable === 0) report(parser, 69, forAwait ? 'await' : 'of');
      nextToken(parser, context, 1);
      right = parseExpression(parser, context, 0);
      consume(parser, context, 17, 1);
      const body = parseStatement(parser, context | 131072, scope, origin, labels, null, 0);
...
function parseSwitchStatement(parser, context, scope, labels, nestedLabels) {
  ...
  while (parser.token !== 16777232) {
    ...
    if (consumeOpt(parser, context, 131150, 1)) {
      ...
      if (seenDefault) report(parser, 0);
      seenDefault = 1;
    }
    consume(parser, context, 22, 1);
...
function parseTryStatement(parser, context, scope, labels) {
  ...
  if (parser.token === 131151) {
    ...
    if (parser.token === 1048588) {
      ...
      const kind = (parser.token & 33554432) === 33554432 ? 256 : 512;
...
function parseExpressionOrLabelledStatement(parser, context, scope, origin, labels, nestedLabels, allowFuncDecl) {
  ...
  if (token === 538181707 && parser.token === 34603028) report(parser, 0);
  if (parser.token === 22) {
    return parseLabelledStatement(
      parser,
      context,
    ...
    switch (parser.token) {
      ...
      case 34603021:
        ...
        while (parser.token & (131072 | 262144 | 2162688)) {
          ...
          const local = consumeOpt(parser, context, 65646, 0) ? parseIdentifier(parser, context) : imported;
          specifiers.push(
            context & 2
              ? {
