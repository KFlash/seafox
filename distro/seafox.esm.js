...
function scanIdentifier(parser, context, source, char) {
  ...
  if (char > 90) return scanIdentifierSlowPath(parser, context, source, value, 0);
...
function scanIdentifierOrKeyword(parser, context, source, char) {
  ...
  const value = source.slice(parser.start, parser.index);
  if (char > 90) return scanIdentifierSlowPath(parser, context, source, value, 1);
...
function scanIdentifierSlowPath(parser, context, source, value, maybeKeyword) {
  ...
  if (maybeKeyword && length >= 2 && length <= 11) {
    ...
    if (context & 1024 && (token === 3473483 || token === 262252)) {
      return 2162808;
    }
    return 2162807;
  }
...
function parseArguments(parser, context, inGroup) {
  ...
  while (parser.token !== 17) {
    args.push(
      parser.token === 15 ? parseSpreadElement(parser, context, inGroup) : parseExpression(parser, context, inGroup)
    );
...
function parseIdentifierOrArrow(parser, context) {
  ...
  parser.assignable = 1;
  if (parser.token === 11) {
    const conjuncted = (parser.flags | 0b00000000000000000000000100000000) ^ 0b00000000000000000000000100000000;
    const scope = {
      parent: {
        parent: void 0,
        type: 2
...
function parseAsyncExpression(parser, context, inNew, allowLHS, canAssign, curStart, curLine, curColumn) {
  ...
  if (parser.newLine === 0) {
    if (parser.token === 1179738) {
      if (allowLHS === 0) report(parser, 86, KeywordDescTable[parser.token & 0xff]);
...
function parseAsyncArrowOrCallExpression(
  ...
  parser.flags = (parser.flags | 0b00000000000000000000010100000000) ^ (0b00000000000000000000010100000000 | 1024);
  let expr = null;
  let conjuncted = 0;
  const params = [];
  while (parser.token !== 17) {
    ...
    if (token & (2162688 | 131072 | 262144)) {
      ...
      if (parser.token === 17 || parser.token === 19) {
        conjuncted |= (parser.assignable === 0 ? 8 | 256 : 0) | ((token & (262144 | 537919488)) !== 0 ? 256 : 0);
      ...
      consume(parser, context, 17, 0);
      parser.flags =
        ((parser.flags | 0b00000000000000000000000000011110) ^ 0b00000000000000000000000000011110) | conjuncted | 8;
      parser.assignable = 0;
      return context & 2
        ? {
            type: 'CallExpression',
...
function parseArrowFunctionAfterParen(
  ...
  parser.flags =
    ((parser.flags | 0b00000000000000000000110000011110) ^ 0b00000000000000000000110000011110) | conjuncted;
  if (canAssign === 0) report(parser, 73);
  let i = params.length;
  while (i--) {
...
function parseUnaryExpression(parser, context, inGroup, inNew, allowLHS, start, line, column) {
  if (allowLHS === 0) report(parser, 0);
  if (inNew === 1) report(parser, 48, KeywordDescTable[parser.token & 0xff]);
...
function parseClassElementList(
  ...
  parser.flags = (conjuncted | 32 | 64) ^ (32 | 64);
...
function parseForStatementWithVariableDeclarations(
  parser,
  context,
  scope,
  _forAwait,
...
function parseSwitchStatement(parser, context, scope, labels, nestedLabels) {
  ...
  while (parser.token !== 16777232) {
    ...
    if (parser.token === 131155) {
      nextToken(parser, context, 1);
      if (seenDefault) report(parser, 101);
      seenDefault = 1;
    }
    consume(parser, context, 22, 1);
