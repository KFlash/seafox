...
function addBlockName(parser, context, scope, name, kind, origin) {
  ...
  if (scope.type & 64 && scope.parent['#' + name] && (scope.parent['#' + name] & 8) !== 8) {
    report(parser, 22, name);
  }
  if (scope.type & 512 && value && (value & 8) !== 8) {
    if ((kind & 1) === 1) {
      scope.scopeError = recordScopeError(parser, 22, name);
    }
  }
...
function parseUnaryExpression(parser, context, inGroup, inNew, allowLHS, start, line, column) {
  if (allowLHS === 0) report(parser, 0);
  if (inNew === 1) report(parser, 48, KeywordDescTable[parser.token & 0b00000000000000000000000011111111]);
...
function parseClassElementList(
  ...
  const kind = (type & 32) === 0 && type & 64 ? 'constructor' : type & 128 ? 'get' : type & 256 ? 'set' : 'method';
  return context & 2
    ? {
...
function parseGetterSetter(parser, context, kind) {
  ...
  consume(parser, context, 17, 0);
  const body = parseFunctionBody(parser, context, scope, void 0, 0, void 0);
  return context & 2
    ? {
...
function parseImportDeclaration(parser, context, scope) {
  ...
  if (parser.token === 1572868) {
    ...
    if (parser.token & (131072 | 262144 | 2162688)) {
      const { start, line, column } = parser;
      if (!isValidIdentifier(context, parser.token)) report(parser, 26);
      if ((parser.token & 537919488) === 537919488) report(parser, 27);
      ...
      case 34603021:
        ...
        while (parser.token & (131072 | 262144 | 2162688)) {
          ...
          if (consumeOpt(parser, context, 65646, 0)) {
            if ((parser.token & 524288) === 524288 || parser.token === 19) {
              report(parser, 105);
            } else {
            ...
            if ((token & 537919488) === 537919488) report(parser, 27);
            local = imported;
          }
