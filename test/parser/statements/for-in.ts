...
describe('Statements - For in', () => {
  ...
  for (const [source, ctx] of [
    ['for (function(){ }[foo];;);', Context.Empty],
    ['for (456[x] in c) d;', Context.Empty],
    ['for (456..x in c) d;', Context.Empty],
    ['for (/foo/[x] in c) d;', Context.Empty],
    ['for (/foo/.x in c) d;', Context.Empty],
    ['for ("foo"[x] in c) d;', Context.Empty],
    ['for ("foo".x in c) d;', Context.Empty],
    ['for (true.x in c) d;', Context.Empty],
    ['for ([][y] <<= p;;) x;', Context.Empty],
    ['for ([].w ^= s;;) x;', Context.Empty],
    ['for(function(){};;)x', Context.Empty],
    ['for(function(){if(x in 3);};;)x', Context.Empty],
    ['for ([].w ^= s;;) x;', Context.Empty],
    ['for ([][y] <<= p;;) x;', Context.Empty],
    ['for ([].u |= c;;) x;', Context.Empty],
    ['for ({}[y] ^= x;;) x;', Context.Empty],
    ['for ({}.u |= c;;) x;', Context.Empty],
    ['for ({}[y] ^= x;;) x;', Context.Empty],
    ['for (const {j} in x) { const [foo] = [j] }', Context.Empty],
    ['for (const {j} in x) { function foo() {return j} }', Context.Empty],
    ['2; for (var b in { x: 0 }) { 3; }', Context.Empty],
    ['2; for (const b in { x: 0 }) { 3; }', Context.Empty],
    ['2; for (let b in { x: 0 }) { 3; }', Context.Empty],
    ['for ( [let][1] in obj ) ;', Context.Empty],
    ['for (x.y in { attr: null }) {}', Context.Empty],
    ['for ((x) in { attr: null }) {}', Context.Empty],
    ['for ( let[x] in obj ) {}', Context.Empty],
    ['for ((a in b).x in {});', Context.Empty],
    ['for (x in null, { key: 0 }) {}', Context.Empty],
    ['for ((let.x) of []) {}', Context.Empty],
    ['for (let a = b => { return b in c; }; ;);', Context.Empty],
    ['for (let a = (b in c); ;);', Context.Empty],
    ['for (let a = (b in c && d); ;);', Context.Empty],
    ['for (let a = b => (b in c); ;);', Context.Empty],
    ['for (let a = ((b in c) && (d in e)); ;);', Context.Empty]
  ]) {
    it(source as string, () => {
      t.doesNotThrow(() => {
        parseScript(source as string, {
